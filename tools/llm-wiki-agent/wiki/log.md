...
## [2026-09-26] ingest | 多智能体协作的工程化(Multi-Agent Orchestration)

Sources 11 → **12**; wiki pages 41 → **43** (1 new source page, 1 new concept page; 1 existing concept page updated additively: [[AgentHarness]] — MultiAgent See Also link).
**Authorisation note:** ingested under explicit user instruction ("继续做", 2026-09-26) continuing the same-day authorised chain; not a line-by-line human check.
**Evidence grade, recorded three times (material / source page / overview):** the team@k numbers trace to AlphaSignal's report of the Microsoft Research + UC Berkeley paper (paper entity and repo `github.com/jerryjonghopark/test-time-communication` given; **arXiv abstract not opened**); Agensh figures are secondhand; Claude Code Projects' shared memory has "no detailed performance evaluation". Direction-and-magnitude grade; the arXiv primary is now a Next Ingest Suggestion on [[overview]].
`raw/`: one file **added** (`multi-agent.html`, byte-identical, md5-verified); nothing in `raw/` edited or deleted.
New pages: `wiki/sources/multi-agent.md` and `wiki/concepts/MultiAgent.md` — the wiki's first **between-agents** page: team@k vs best@k (ARC-AGI-3 team@5 ≈ best@33; LP85 64-all-failed vs team@5 65%), verified-progress sharing as the mechanism, the verifier as the precondition ("fails without verifier or low compute"), cross-contamination (互相污染) as the new failure surface extending [[Provenance]], Claude Code Projects' coordinator pattern with self-declared limits, Agensh decentralised scale.
`wiki/index.md`: +1 Sources entry, +1 Concepts entry. `wiki/overview.md`: Cluster E extended with the [[MultiAgent]] bullet and retitled "the loop's host, where it runs, who it works with"; RUNTIME SIDE diagram gains the collaboration leg; the eleven-page story became the **twelve-page** story; one new cross-cutting theme ("the verifier is the precondition of collaboration" — the first wiki entry whose boundary condition *is* the content); open question "multi-agent memory sharing" updated (orchestration side covered; governance/RBAC side remains open); arXiv primary added to Next Ingest Suggestions.
No dangling `[[wikilink]]` introduced: new links target pages that now exist ([[MultiAgent]], the new source slug) or already existed ([[Compaction]], [[Provenance]], [[AgentHarness]], [[ContextEngineering]], [[Agent]], [[Anthropic]], [[OpenAI]]).
Outside the wiki, the repo-level `concept-relationship.md` gained "扩展五 · 多智能体协作" (shared-workspace diagram with the win/lose fork on the verifier; the Compaction subagent footnote promoted to mainline; provenance extended to peer-conclusion verification status).
With this ingest, all three 2026-09-26 radar candidates (harness / sandbox / multi-agent — the three runtime layers) have been materialised, ingested and graphed **on the same day**, under explicit user instruction.
